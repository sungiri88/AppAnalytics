...
namespace AppAnalytics
{
    public class Apple
    {
        ...
        {
            string APIurl = ConfigurationManager.AppSettings["AppleAPI"] + DeviceID + ".json";
            ...
            //webRequest.Proxy = new WebProxy("https://api.apptweak.com");
        ...
        public DataTable GetMasterAppData(string JSONString, string appID = "")
        {
            ...
            foreach (JToken item in ReviewArray)
            {
                ...
                string review = (string)item["content"];
        ...
        public AppInformation GetAppInformation(string JSONString)
        {
            ...
            appInfo.Email = "appsupport@hughestelematics.com";
            decimal _Rating = (decimal)token.SelectToken("content.ratings.all_versions.average");
            appInfo.Rating = Math.Round(_Rating, 3);
            appInfo.ReviewCount = (int)token.SelectToken("content.ratings.all_versions.count");
            appInfo.OneStarCount = (int)token.SelectToken("content.ratings.all_versions.star_count.1");
            appInfo.TwoStarCount = (int)token.SelectToken("content.ratings.all_versions.star_count.2");
            appInfo.ThreeStarCount = (int)token.SelectToken("content.ratings.all_versions.star_count.3");
            appInfo.FourStarCount = (int)token.SelectToken("content.ratings.all_versions.star_count.4");
            appInfo.FiveStarCount = (int)token.SelectToken("content.ratings.all_versions.star_count.5");
            return appInfo;
        }
    }
}